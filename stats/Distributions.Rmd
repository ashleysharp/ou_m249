...
n_ <- 50 #finite range, Bernoulli if n = 1
p <- 49/50
# p <- l/n_ 
...
l <- 1.05 #constant event rate
t <- 1 #time interval
lt <- l*t #decreasing p.m.f. when lt < 1
str_c("rate = ", l, ", time = ", t, ", lt = ", lt)
#Range
range <- 0:14 #unbounded to the right
#p.d.f
dp <- function(x, lt){
...
p <- 0.5
str_c("p = ", p)
range <- 1:20 #1,2,3,..., unbounded to the right
#p.m.f.
dg <- function(x, p){
        ((1-p)^(x-1))*p
}
...
#Parameters
l <- 2 #rate
range <- seq(0.0001, 5, 5/1000) # X > 0, unbounded to the right
str_c("lambda = ", l)
...
To do Non-linear regression models and transformations.\
