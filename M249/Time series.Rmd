...
```{r}
df_infections <- read_sav("data/Book 2/infections.sav") %>%
...
## Salmonella
```{r}
ggplot(df_infections, aes(x = date, y = salmonella)) + 
  geom_line() + 
  scale_x_date(
    breaks = make_date(year = seq(year(min(df_infections$date)),
                                  year(max(df_infections$date)),
                                  by = 1),
                       month = 1, day = 1),
    date_labels = "%Y"
  ) +
...
## Rotavirus
```{r}
ggplot(df_infections, aes(x = date, y = rotavirus)) + 
  geom_line() + 
  scale_x_date(
    breaks = make_date(year = seq(year(min(df_infections$date)),
                                  year(max(df_infections$date)),
...
ggplot(df_infections, aes(x = date, y = log_rotavirus)) + 
  geom_line() + 
  scale_x_date(
    breaks = make_date(year = seq(year(min(df_infections$date)),
                                  year(max(df_infections$date)),
...
## Airline passengers
```{r}
df_airline <- read_sav("data/Book 2/airline.sav")
```
