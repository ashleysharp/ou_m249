...
## Airline passengers
```{r}
ts_airline <- read_sav("data/Book 2/airline.sav") %>% clean_names() %>%
...
```{r}
ggplot(ts_airline, aes(x = date, y = passengers)) + 
  geom_line() + 
  scale_x_date(
    breaks = make_date(year = seq(year(min(ts_airline$date)),
                                  year(max(ts_airline$date)),
                                  by = 2),
                       month = 1, day = 1),
    date_labels = "%b%Y"
  ) + theme_minimal() +
  theme(
    axis.text.x = element_text(
      ...
      hjust = 1,        # right-justify so they align nicely
      vjust = 1
    )
  )

ggplot(ts_airline, aes(x = date, y = log_passengers)) + 
  geom_line() + 
  scale_x_date(
    breaks = make_date(year = seq(year(min(ts_airline$date)),
                                  year(max(ts_airline$date)),
...
ggplot(ts_airline, aes(x = date, y = sqrt_passengers)) + 
  geom_line() + 
  scale_x_date(
    breaks = make_date(year = seq(year(min(ts_airline$date)),
                                  year(max(ts_airline$date)),
...
ggplot(ts_airline, aes(x = date, y = qrt_passengers)) + 
  geom_line() + 
  scale_x_date(
    breaks = make_date(year = seq(year(min(ts_airline$date)),
                                  year(max(ts_airline$date)),
