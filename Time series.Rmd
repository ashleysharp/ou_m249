...
```{r}
components(fit_airline5)
```

```{r}
augment(fit_airline5)
```
...
```{r}
components(fit_infections2)
```

```{r}
augment(fit_infections2)
```
...
### Differencing
```{r}
ts_production_diff <- ts_production |> mutate(
  d1_index = difference(index),
  d2_index = difference(d1_index))
```

```{r}
ggplot(ts_production_diff, aes(x = year_quarter, y = index)) + 
  geom_line() + 
  theme_minimal()

ggplot(ts_production_diff, aes(x = year_quarter, y = d1_index)) + 
  geom_line() + 
  theme_minimal()

ggplot(ts_production_diff, aes(x = year_quarter, y = d2_index)) + 
  geom_line() + 
  theme_minimal()
```
## FTSE100
...
```{r}
ts_ftse <- read_sav("data/Book 2/ftse100.sav") |> 
  clean_names() |>
  mutate(year_month = yearmonth(sprintf("%d-%02d", year, month))
  ) |>
  as_tsibble(index = year_month) |>
  mutate(
    d1_ftse = difference(ftse),
    d1_logftse = difference(logftse)
  )
```

```{r}
ggplot(ts_ftse, aes(x = year_month, y = ftse)) + 
  geom_line() + 
  theme_minimal()

ggplot(ts_ftse, aes(x = year_month, y = d1_ftse)) + 
  geom_line() + 
  theme_minimal()

ggplot(ts_ftse, aes(x = year_month, y = logftse)) + 
  geom_line() + 
  theme_minimal()

ggplot(ts_ftse, aes(x = year_month, y = d1_logftse)) + 
  geom_line() + 
  theme_minimal()
```
...
```{r}
ts_airline5_diff <- ts_airline5 |>
  mutate(d1_passengers = difference(passengers),
         log_passengers = log(passengers),
         d1_log_passengers = difference(log_passengers),
         d2_log_passengers = difference(d1_log_passengers))
```

```{r}
ggplot(ts_airline5_diff, aes(x = year, y = passengers)) + 
  geom_line() + 
  theme_minimal()

ggplot(ts_airline5_diff, aes(x = year, y = d1_passengers)) + 
  geom_line() + 
  theme_minimal()

ggplot(ts_airline5_diff, aes(x = year, y = log_passengers)) + 
  geom_line() + 
  theme_minimal()

ggplot(ts_airline5_diff, aes(x = year, y = d1_log_passengers)) + 
  geom_line() + 
  theme_minimal()

ggplot(ts_airline5_diff, aes(x = year, y = d2_log_passengers)) + 
  geom_line() + 
  theme_minimal()
...
```{r}
ts_electricity_diff = ts_electricity |>
  mutate(log_sademand = log(sademand),
         d1_log_sademand = difference(log_sademand))
```

```{r}
ggplot(ts_electricity_diff, aes(x = year_month, y = sademand)) + 
  geom_line() + 
  theme_minimal()

ggplot(ts_electricity_diff, aes(x = year_month, y = log_sademand)) + 
  geom_line() + 
  theme_minimal()

ggplot(ts_electricity_diff, aes(x = year_month, y = d1_log_sademand)) + 
  geom_line() + 
  theme_minimal()
```
...
### ACF
```{r}
ts_ftse |> ACF(d1_logftse, lag_max = 20) |> autoplot() + theme_minimal()
```

```{r}
ts_ftse |> features(d1_logftse, ljung_box, lag = 20)
```

### PACF
```{r}
ts_ftse |> PACF(d1_logftse, lag_max = 20) |> autoplot() + theme_minimal()
```

The stationary series is uncorrelated so a suitable model for log ftse is ARIMA(0,1,0)

## UK index of production

```{r}
ACF(ts_production_diff, d2_index) |> autoplot() + theme_minimal()
ACF(ts_production_diff, d2_index) |> autoplot() + theme_minimal()
```
```{r}
fit_production <- ts_production  |>
  model(ses = ETS(index ~ error("A") + trend("N") + season("N")),
        holt = ETS(index ~ error("A") + trend("A") + season("N")),
        arima021 = ARIMA(index ~ pdq(0, 2, 1)),
        arima120 = ARIMA(index ~ pdq(1, 2, 0)))
```
...
```{r}
ggplot(augment(fit_production) |> filter(.model == "arima021"), aes(x = year_quarter, y = .innov)) + geom_line() + theme_minimal()

ggplot(augment(fit_production) |> filter(.model == "arima021"), aes(x = .innov)) + geom_histogram() + theme_minimal()

augment(fit_production) |> filter(.model == "arima021") |> ACF(.innov, lag_max = 20) |> autoplot() + theme_minimal()
```

### gg_tsdisplay()
```{r}
augment(fit_production) |>
...
```{r}
autoplot(fc_production, ts_production) + theme_minimal()
```

## Electricity
```{r}
fit_electricity <- ts_electricity |>
  model(holt = ETS(log(sademand) ~ error("A") + trend("A") + season("N")),
        arima111 = ARIMA(log(sademand) ~ pdq(1,1,1)))
```

```{r}
fit_electricity_long <- augment(fit_electricity) |>
  pivot_longer(cols = c(sademand, .fitted),
               names_to = "legend")
...
```
```{r}
ggplot(augment(fit_electricity) |> filter(.model == "arima111"), aes(x = year_month, y = .innov)) + geom_line() + theme_minimal()

ggplot(augment(fit_electricity) |> filter(.model == "arima111"), aes(x = .innov)) + geom_histogram() + theme_minimal()

augment(fit_electricity) |> filter(.model == "arima111") |> ACF(.innov, lag_max = 20) |> autoplot() + theme_minimal()
```
```{r}
augment(fit_electricity) |> features(.innov, ljung_box, lag = 18)
```
...
## Airline passengers
```{r}
ACF(ts_airline5_diff, d2_log_passengers) |> autoplot() + theme_minimal()
PACF(ts_airline5_diff, d2_log_passengers) |> autoplot() + theme_minimal()
```
...
```{r}
fit_airline5_long <- augment(fit_airline5) |>
  pivot_longer(cols = c(passengers, .fitted),
               names_to = "legend")
...
```{r}
ggplot(augment(fit_airline5), aes(x = year, y = .innov)) + geom_line() + theme_minimal()

ggplot(augment(fit_airline5), aes(x = .innov)) + geom_histogram() + theme_minimal()

augment(fit_airline5) |> ACF(.innov, lag_max = 20) |> autoplot() + theme_minimal()
```
```{r}
augment(fit_airline5) |> features(.innov, ljung_box, lag = 18)
```
...
```{r}
n = 100
random_walk <- tibble(t = 1:n, 
                      eps = rnorm(n), 
                      y = cumsum(eps)) %>%
  as_tsibble(index = t) %>%
  mutate(d1 = difference(y))

ggplot(random_walk, aes(x = t, y = y)) + geom_line() + theme_minimal()
ggplot(random_walk, aes(x = t, y = d1)) + geom_line() + theme_minimal()
```
...
n <- 200
phi <- 0.8
y <- arima.sim(model = list(order = c(1, 0, 0), ar = phi), n = n)
plot.ts(y)
acf(y)
pacf(y)
```
...
phi1 <- 0.6
phi2 <- -0.5
y <- arima.sim(model = list(order = c(2, 0, 0), ar = c(phi1, phi2)), n = n)
plot.ts(y)
acf(y)
pacf(y)
```
...
n <- 200
phi <- 0.8
y <- arima.sim(model = list(order = c(1, 1, 0), ar = phi), n = n)
plot.ts(y)
acf(diff(y))
pacf(diff(y))
```
...
phi1 <- 0.6
phi2 <- -0.3
y <- arima.sim(model = list(order = c(2, 1, 0), ar = c(phi1, phi2)), n = n)
plot.ts(y)
acf(diff(y))
pacf(diff(y))
```
...
n <- 200
phi <- 0.6
y <- arima.sim(model = list(order = c(1, 2, 0), ar = phi), n = n)
plot.ts(y)
acf(diff(y, differences = 2))
pacf(diff(y, differences = 2))
```
...
phi1 <- 0.6
phi2 <- -0.3
y <- arima.sim(model = list(order = c(2, 2, 0), ar = c(phi1, phi2)), n = n)
plot.ts(y)
acf(diff(y, differences = 2))
pacf(diff(y, differences = 2))
```
...
n <- 200
theta <- 0.5
y <- arima.sim(model = list(order = c(0, 0, 1), ma = theta), n = n)
plot.ts(y)
acf(y)
pacf(y)
```
...
theta1 <- 0.7
theta2 <- -0.4
y <- arima.sim(model = list(order = c(0, 0, 2), ma = c(theta1, theta2)), n = n)
plot.ts(y)
acf(y)
pacf(y)
```
...
n <- 200
theta <- 0.5
y <- arima.sim(model = list(order = c(0, 1, 1), ma = theta), n = n)
plot.ts(y)
acf(diff(y))
pacf(diff(y))
```
...
theta1 <- 0.7
theta2 <- -0.4
y <- arima.sim(model = list(order = c(0, 1, 2), ma = c(theta1, theta2)), n = n)
plot.ts(y)
acf(diff(y))
pacf(diff(y))
```
...
n <- 200
theta <- 0.5
y <- arima.sim(model = list(order = c(0, 2, 1), ma = theta), n = n)
plot.ts(y)
acf(diff(y, differences = 2))
pacf(diff(y, differences = 2))
```
...
theta1 <- 0.7
theta2 <- -0.4
y <- arima.sim(model = list(order = c(0, 2, 2), ma = c(theta1, theta2)), n = n)
plot.ts(y)
acf(diff(y, differences = 2))
pacf(diff(y, differences = 2))
```
...
phi <- 0.8
theta <- 0.4
y <- arima.sim(model = list(order = c(1, 1, 1), ar = phi, ma = theta), n = n)
plot.ts(y)
acf(diff(y))
pacf(diff(y))
```
...
phi <- 0.8
theta <- 0.4
y <- arima.sim(model = list(order = c(1, 2, 1), ar = phi, ma = theta), n = n)
plot.ts(y)
acf(diff(y, differences = 2))
pacf(diff(y, differences = 2))
```