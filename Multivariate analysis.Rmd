...
```{r}
scores <- as.data.frame(predict(fit)$x) |>
  mutate(type = banknotes$type)
...
banknotes_ld <- banknotes |> 
  mutate(LD1 = scores[, 1],
         allocated = case_when(LD1 > 0 ~ 2,
                               LD1 < 0 ~ 1),
         class = class,
         valid = allocated == type)
```

```{r}
# Note: class equivalent to allocation rule
banknotes_ld |> count(allocated, class)
```

```{r}
banknotes_ld |> 
  count(allocated, type) |> 
...
```{r}
# Confusion matrix (training set)
table(Truth = banknotes$type, Pred = pred$class)
```

```{r}
# Accuracy
mean(pred$class == as.numeric(banknotes$type))
```
